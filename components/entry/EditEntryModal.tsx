    ...
    <Modal open={open} onClose={onClose} title="Editar lançamento">
      <div className="px-1 space-y-4">
        <div className="flex items-center gap-6">
          <button onClick={() => handleKindChange('despesa')} className={`pb-2 transition-colors ${entry.kind === 'despesa' ? 'text-rose-400 border-b-2 border-rose-400' : 'text-zinc-400 hover:text-zinc-200'}`}>Despesa</button>
          <button onClick={() => handleKindChange('receita')} className={`pb-2 transition-colors ${entry.kind === 'receita' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-zinc-400 hover:text-zinc-200'}`}>Receita</button>
        </div>
        
        <div className="bg-zinc-800 rounded-xl p-3">
          <div className="text-zinc-400 text-sm">{entry.kind === 'receita' ? 'Valor da Receita' : 'Valor da Despesa'}</div>
        ...
        <div>
          ...
          <input className="w-full px-3 py-2 rounded-xl bg-zinc-800 border border-zinc-700 outline-none focus:border-cyan-500" value={entry.description} onChange={(e) => handleChange('description', e.target.value)} />
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {availableCategories.map((cat) => (
              <button key={cat.id} onClick={() => handleChange('category', cat.id)} className={`flex items-center gap-2 p-2 rounded-lg border text-left transition-colors ${entry.category === cat.id ? "bg-cyan-600/20 border-cyan-500 text-cyan-300" : "bg-zinc-800 border-zinc-700 hover:border-zinc-600"}`}>
        ...
        <div>
          ...
          <input type="date" className="w-full px-3 py-2 rounded-xl bg-zinc-800 border border-zinc-700 outline-none focus:border-cyan-500" value={entry.dueDate} onChange={(e) => handleChange('dueDate', e.target.value)} />
        </div>
        ...
        {(entry.recurrence === 'always' || entry.recurrence === 'parcelado') && (
          <div>
            ...
            <select value={entry.frequency} onChange={(e) => handleChange('frequency', e.target.value as Frequency)} className="w-full px-3 py-2 rounded-xl bg-zinc-800 border border-zinc-700 outline-none focus:border-cyan-500">
        ...
        {entry.recurrence === 'parcelado' && (
          <div>
            ...
            <input type="number" min={1} value={entry.parcels} onChange={(e) => handleChange('parcels', parseInt(e.target.value || "1"))} className="w-full px-3 py-2 rounded-xl bg-zinc-800 border border-zinc-700 outline-none focus:border-cyan-500" />
          </div>
        )}
        ...
        <div className="pt-2 flex gap-2">
          ...
          <button onClick={onClose} className="px-4 rounded-xl bg-zinc-800 hover:bg-zinc-700">Cancelar</button>
        </div>
      </div>
    </Modal>
  );
