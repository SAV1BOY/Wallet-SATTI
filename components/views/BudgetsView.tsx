...
const BudgetsView: React.FC<BudgetsViewProps> = ({ allOccurrences, cursor, data, onEdit, settings, categories }) => {
    ...
    const budgetData = useMemo(() => {
        ...
        return categories.despesa
            ...
            .map(cat => {
                const key = `categories.despesa.${cat.id}`;
    ...
    }, [monthlyExpensesByCategory, data.budgets, monthStr, categories.despesa, t]);

    const totals = useMemo(() => {
        const result = budgetData.reduce((acc, item) => {
            acc.spent += item.spent;
            acc.budgeted += item.budgeted;
            return acc;
        }, { spent: 0, budgeted: 0 });

        const percentage = result.budgeted > 0 ? (result.spent / result.budgeted) * 100 : (result.spent > 0 ? 101 : 0);
        ...
        <div className="space-y-6">
            <div className="bg-white dark:bg-zinc-900 rounded-2xl p-4 border border-zinc-200 dark:border-zinc-800">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">{t('budgets.monthBudget')}</h3>
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                        <div className="text-xs text-zinc-500 dark:text-zinc-400">{t('budgets.budgeted')}</div>
                        <div className="text-lg font-semibold">{fmtMoney(totals.budgeted, settings.currency, locale)}</div>
                    </div>
                    <div>
                        <div className="text-xs text-zinc-500 dark:text-zinc-400">{t('budgets.spent')}</div>
                        <div className="text-lg font-semibold">{fmtMoney(totals.spent, settings.currency, locale)}</div>
                    </div>
                    <div>
                        <div className="text-xs text-zinc-500 dark:text-zinc-400">{t('budgets.remaining')}</div>
                        <div className={`text-lg font-semibold ${totals.budgeted - totals.spent >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                            {fmtMoney(totals.budgeted - totals.spent, settings.currency, locale)}
                        </div>
                    </div>
                </div>
                {totals.budgeted > 0 && (
                    <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                        <div className="w-full bg-zinc-200 dark:bg-zinc-700 rounded-full h-4 relative overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-500 ${getProgressBarClasses(totals.percentage)}`}
                                style={{ width: `${Math.min(totals.percentage, 100)}%` }}
                            ></div>
                        </div>
                        <div className="text-sm text-zinc-500 dark:text-zinc-400 text-center mt-1.5"
                            dangerouslySetInnerHTML={{ __html: t('budgets.usedPercentage', { percentage: totals.percentage.toFixed(0) }) }}
                        />
                    </div>
                )}
            </div>
                ...
                {budgetData.map(item => (
                    <div key={item.id} className="bg-white dark:bg-zinc-900 rounded-2xl p-4 border border-zinc-200 dark:border-zinc-800">
                        <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="text-right flex-shrink-0 ml-2">
                                <div className={`text-lg font-bold ${item.remaining >= 0 ? 'text-zinc-800 dark:text-zinc-200' : 'text-rose-500'}`}>
                                    {fmtMoney(item.remaining, settings.currency, locale)}
                                </div>
                                <div className="text-xs text-zinc-500 dark:text-zinc-400">
                                    {item.remaining >= 0 ? t('common.remaining') : t('common.overspent')}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="text-sm text-zinc-500 dark:text-zinc-400 flex justify-between mt-1.5">
                            <span className="font-medium text-zinc-800 dark:text-zinc-200">{fmtMoney(item.spent, settings.currency, locale)}</span>
                            <span>{t('common.of')} {fmtMoney(item.budgeted, settings.currency, locale)} ({item.percentage.toFixed(0)}%)</span>
                        </div>
                    </div>
                ))}
                {budgetData.length === 0 && (
                    <div className="text-center py-16 text-zinc-500 dark:text-zinc-400">
                        <p className="mb-6">{t('budgets.noBudgets')}</p>
